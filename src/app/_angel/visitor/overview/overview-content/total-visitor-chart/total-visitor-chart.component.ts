...
export class TotalVisitorChartComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        this.chartOptions = {
            ...
            yaxis: {
                ...
                labels: {
                    formatter: val => Math.round(val).toString(),  // tam sayı formatı
                    style: {
    ...
    getChartData(period: '1D' | '1W' | '1M') {
        ...
        this.profileService.requestMethod(sp, { noloading: 'true' })
            ...
            .subscribe((res: any) => {
                const response = res[0].x;
                const message = res[0].z;

                if (message.islemsonuc == -1) return;

                console.log("Response Data:", response);
                const entries = Object.entries(response[0]) as [string, number][];
                const dates = entries.map(([date]) => date);
                const values = entries.map(([, value]) => value);

                this.periodDates[period] = dates;
                this.periodData[period] = values;
