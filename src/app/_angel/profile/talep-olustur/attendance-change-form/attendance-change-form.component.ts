...
export class AttendanceChangeFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.isFromAttendance) {
      ...
      if (this.gridStartDate == this.gridEndDate && this.selectedEmployeesFromAttendance.length <= 1) {
        this.attendanceForm.addControl('shiftDate', this.formBuilder.control('', Validators.required));
      }
    } else {
      this.attendanceForm.addControl('shiftDate', this.formBuilder.control('', Validators.required));
    }
  ...
  getDeviceList() { // Terminalleri Almak İçin API'ye İstek 
    this.profileService.getTypeValues('terminaller').pipe(takeUntil(this.ngUnsubscribe)).subscribe((response: ResponseModel<OKodFieldsModel, ResponseDetailZ>[]) => {
  ...
  getSelectedRows() {
    ...
    console.log("Giriş-Çıkış Kaydı Bulunan Var Mı? ", this.selectedEmployeesFromAttendance.some((p:any) => p.ggiris != null || p.ggiris != undefined));
    
  }

  isThereData():boolean {
    return this.selectedEmployeesFromAttendance.some((p:any) => p.ggiris != null || p.ggiris != undefined);
  }
