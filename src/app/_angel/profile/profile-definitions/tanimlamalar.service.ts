...
export class TanimlamalarService {
  ...
  clearDailyMenu(
    ...
    console.log('cleear',sp);
    return this.requestMethod(sp);
  }

  getDailyMenu(tarih: any, lokasyon?: any, menu?: any) {
    var sp: any[] = [
      {
        mkodu: 'yek084',
        tarih: tarih,
  ...
  requestMethod(sp: any) {
    ...
    var encryptedParam = CryptoES.AES.encrypt(
      CryptoES.enc.Utf8.parse(
        this.helperService.gateResponseY + JSON.stringify(sp)
      ),
      key,
      {
        // keySize : 128 / 8,
    ...
    return this.httpClient.get<any>(
      this.apiUrlService.apiUrl + '/process',
      options
    );
  }
}
