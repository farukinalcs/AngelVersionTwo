...
export class AuthorityRolesComponent implements OnInit, OnDestroy {
    ...
    onCheckboxChange() {
        const result = this.keys.reduce((acc, key) => {
            acc[key] = this.checkboxData[key].map((val: any) => (val.checked ? "1" : "0")).join('');
            return acc;
        }, {});

        console.log("Gönderilecek veri:", result);
        ...
        var sp: any[] = [
            {
                ...
                ziyaretci: result['Ziyaretci'],
                yemekhane: result['Yemekhane'],
                kantin: result['Kantin'],
                otopark: result['Otopark']
            }
        ];

        console.log("Rol Yetki Değiştir Param : ", sp);
        ...
        this.profileService
            ...
            .subscribe((response: any) => {
                ...
                console.log("Rol Yetki Değiştirildi : ", data);

                this.toastrService.success(
                    this.translateService.instant('Yetkiler_Değiştirildi'),
