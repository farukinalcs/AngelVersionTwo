...
@Component({
  ...
  <div class="card shadow-sm mb-3" style="border: 1px solid #ed1b24;">
    <!-- Header -->
    <div class="card-header d-flex align-items-center gap-2 sticky-top bg-danger" style="z-index:0!important">
      ...
      <div class="ms-auto d-flex align-items-center gap-2">
        ...
        >
          <option *ngFor="let s of pageSizeOptions" [value]="s">{{ s }}/sayfa</option>
        </select>
      </div>
    </div>
    ...
    <ng-template #tableTpl>
      ...
      <div class="table-responsive d-none d-sm-block">
        <table class="table table-sm align-middle mb-0">
          <thead class="table-light" style="position: sticky; top: 0; z-index: 0;">
            <tr>
              <th *ngFor="let c of columns"
                ...
                <button *ngIf="c.sortable; else plain" class="btn btn-link btn-sm text-decoration-none p-0"
                        (click)="toggleSort(c.key)">
                  <span>{{ c.header }}</span>
                  <small class="opacity-75 ms-1" *ngIf="sortKey()===c.key">{{ sortDir()==='asc' ? '▲' : '▼' }}</small>
                </button>
          ...
          <tbody>
            <tr *ngFor="let row of pageRows(); let i = index; trackBy: trackByIndex" class="table-row-hover">
              <td *ngFor="let c of columns"
                ...
                <ng-container [ngSwitch]="c.type">
                  ...
                  <ng-container *ngSwitchDefault>
                    <span (click)="onCellClick(row[c.key], row, c)"
            ...
            <tr *ngIf="pageRows().length === 0">
              <td [attr.colspan]="columns.length" class="text-center text-muted py-4">Kayıt bulunamadı</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="d-block d-sm-none">
        <div *ngFor="let row of pageRows(); trackBy: trackByIndex" class="border-bottom p-3">
          <div class="row g-2">
            <div *ngFor="let c of columns" [class.d-none]="c.hideOnMobile">
              <div class="text-muted small">{{ c.header }}</div>
              <div class="fw-medium">
                <ng-container [ngSwitch]="c.type">
                  <span *ngSwitchCase="'date'">{{ formatDate(row[c.key], c) }}</span>
        ...
        <div *ngIf="pageRows().length === 0" class="text-center text-muted p-3">Kayıt bulunamadı</div>
      </div>
    </ng-template>

    <!-- Footer -->
    <div class="card-footer d-flex align-items-center gap-2">
      <div class="text-muted small">{{ startIndex()+1 }}–{{ endIndex() }} / {{ total() }}</div>
      <div class="ms-auto d-flex align-items-center gap-2">
        <button class="btn btn-outline-secondary btn-sm"
                (click)="prevPage()" [disabled]="pageIndex()===0">Önceki</button>
        <div class="small">{{ pageIndex()+1 }} / {{ totalPages() }}</div>
        <button class="btn btn-outline-secondary btn-sm"
                (click)="nextPage()" [disabled]="pageIndex()+1>=totalPages()">Sonraki</button>
      </div>
    </div>
  </div>
  `,
...
@property --border-angle {
  ...
  :host { display: block; }
  .table-row-hover:hover { background: rgba(0,0,0,0.03); }
  .skeleton {
    height: 36px; border-radius: .5rem;
...
export class ModernTableComponent {
  ...
  @Input() set rows(value: any[]) { this._rows.set(Array.isArray(value) ? value : []); }
  ...
  @Output() dateClick = new EventEmitter<{ value: Date; row: any; key: string }>();
  @Output() cellClick = new EventEmitter<{ value: any; row: any; col: ColumnDef }>();
  ...
  filtered = computed(() => {
    ...
    return this._rows().filter(r => keys.some(k => String(r?.[k] ?? '').toLowerCase().includes(q)));
  });
  ...
  sorted = computed(() => {
    ...
    return [...this.filtered()].map((v,i)=>[v,i] as const).sort((a,b)=>{
      const av=a[0]?.[key], bv=b[0]?.[key];
  ...
  startIndex = computed(()=> Math.min(this.pageIndex()*this.pageSize(), Math.max(0, this.total()-1)));
  endIndex = computed(()=> Math.min(this.startIndex()+this.pageSize(), this.total()));
  pageRows = computed(()=> this.sorted().slice(this.startIndex(), this.endIndex()));
  ...
  toggleSort(key: string) {
    ...
    else { this.sortDir.set(this.sortDir()==='asc' ? 'desc' : 'asc'); }
    this.sortChange.emit({ key, dir: this.sortDir() });
  }
  prevPage() { if (this.pageIndex()>0){ this.pageIndex.set(this.pageIndex()-1); this.pageChange.emit({ index:this.pageIndex(), size:this.pageSize()}); } }
  nextPage() { if (this.pageIndex()+1<this.totalPages()){ this.pageIndex.set(this.pageIndex()+1); this.pageChange.emit({ index:this.pageIndex(), size:this.pageSize()}); } }
  ...
  formatCell(v: unknown): string {
    ...
    if (v instanceof Date) return new Intl.DateTimeFormat('tr-TR', {
