...
export class AttendanceChangeComponent implements OnInit, OnDestroy {
    ...
    stepperFields: any[] = [
        {
            ...
            number: 1,
            title: this.translateService.instant('Yapılacak İşlem'),
            desc: 'İşlem yapmak istediğiniz alanı seçiniz',
        },
        {
            class: 'stepper-item',
            number: 2,
            title: this.translateService.instant('Seçili Personel'),
            desc: 'Seçili personel için işlem yapabilirsiniz',
        },
        {
            class: 'stepper-item',
            number: 3,
            title: this.translateService.instant('Ek Bilgiler'),
            desc: 'Eklenmesi gereken bilgileri giriniz',
        },
        {
            class: 'stepper-item',
            number: 4,
    ...
    getTooltipScript(): any[] {
        ...
        if (uniquePersons.length === 1) {
            ...
            if (personsLength == 1) {
                ...
                firstPerson = `${uniquePersons[0].ad} ${uniquePersons[0].soyad}, ${uniquePersons[1].ad
    ...
    createFormGroup() {
        this.attendanceForm = this.formBuilder.group({
            ...
            formState: ['0', Validators.required],
    ...
    listenFormState() {
        this.attendanceForm
            ...
            .subscribe((value: any) => {
                ...
                this.store.dispatch(resetWizardForm());
    ...
    get actionType(): string {
        if (this.attendanceForm.get('formState')?.value == 0) {
            return 'Yeni Hareket Ekleme';
        } else if (this.attendanceForm.get('formState')?.value == 1) {
            return 'Mevcut Hareketi Kaldırma Veya Aktif Etme';
        }
        else if (this.attendanceForm.get('formState')?.value == 2) {
            return 'Hareket Yön Değiştirme';
        } else {
            return 'Giriş-Çıkış Değişiklik Talebi';
        }
    }

    viewDetails(item: any, action: string) {
        this.displayAction = true;
        this.selectedEmployee = item;
        this.selectedAction = action;
    }

    getAllStore(formValues: any) {
        this.store.select(selectAllPersonnelRequests).pipe(take(1)).subscribe(personnelRequests => {
            console.log('Form gönderilecek veri:', personnelRequests);

            var sp: any[] = [];

            if (this.attendanceForm.get('formState')?.value == 2) {

                personnelRequests.forEach((item: any) => {
                    item.changeMovements.forEach((movement: any) => {
                        sp.push({
                            mkodu: 'yek109',
                            tarih: '2000-10-22',
                            terminalid: '0',
                            ...
                            sicilid: '0',
                            sil: '0',
                            hareketid: movement.id.toString(),
                            kaynak: 'u',
                        });
                    });
                });
